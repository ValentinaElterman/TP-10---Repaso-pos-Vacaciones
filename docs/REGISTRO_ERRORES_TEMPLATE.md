...
|---|---------|---------------------|--------------------|-------------------|
| 15 | 	src/routes/adminRoutes.js | GET /all no tiene ningún middleware de autenticación/autorización — cualquiera sin loguearse puede listar todos los usuarios (con hash de password incluido) | pegandole a /api/users/all sin token devolvía la lista completa igual | agregamos authMiddleware más nuevo requireAdmin que valida role === "admin" |
|---|---------|---------------------|--------------------|-------------------|
| 16 | src/controllers/adminController.js | listUsers devuelve el array users completo, exponiendo el hash de contraseña de todos | la respuesta JSON incluía password en cada usuario | limpiamos el array sacando password antes de responder |
|---|---------|---------------------|--------------------|-------------------|
| 17 | src/app.js | no se incluyó el middleware parser de JSON (app.use(express.json())) | cualquier endpoint que intente leer el body (req.body) en los endpoints POST (/register, /login) o PUT (/me) tira "TypeError: Cannot destructure property ... of req.body as it is undefined o lee req.body como undefined" | agregamos app.use(express.json()); en app.js antes de definir las rutas |
|---|---------|---------------------|--------------------|-------------------|
| 18 | src/app.js | en app.js se usa tanto userRoutes como adminRoutes bajo la misma ruta base /api/users | revisando la arquitectura REST en app.js. | cambiamos adminRoutes bajo su propio prefijo: app.use("/api/admin", adminRoutes); (lo cual cambiaría el endpoint a /api/admin/all) |
